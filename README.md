...
### 2026-09-16
- 구인공고 목록: 리스트 행을 카드형으로 개편 (시설명·위치·태그칩·D-day 표시), 뷰 토글 제거, 근무시간대 필터 shift 기준으로 수정
- jobs.js 전체 데이터에 shift(주간/야간/단기)·displayTags 필드 추가, shift 배지 스타일 개선 (urgent D-day 빨간색)
- 인재정보 목록: 뷰 토글 제거 후 리스트 고정, result bar 단순화
- 공고목록·인재정보 히어로 영역 텍스트 시작점을 메인 컬럼 위치에 맞춰 좌측 여백 추가
- 근무시간대 필터 칩 pill 스타일로 변경 (활성=파란 배경+흰 글씨)
- AuthGuard 카드 테두리 핑크(#f0e0ee) → 블루(#d0e4fb)로 수정
